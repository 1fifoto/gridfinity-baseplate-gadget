...
M.MACHINED_BOTTOM_OPENING_MM = M.MID_OPENING_MM
M.MACHINED_BOTTOM_RADIUS_MM = M.MID_RADIUS_MM
...
function M.machining_profile_dimensions_at_depth_mm(depth_mm, cell_width_mm, cell_height_mm)
  local machined_depth = math.min(depth_mm, M.LOWER_START_DEPTH_MM)
  return M.profile_dimensions_at_depth_mm(machined_depth, cell_width_mm, cell_height_mm)
end
...
function M.validate_tool_geometry(rough_dia, finish_dia, vbit_dia, vbit_angle,
  ...
  if finish_dia * 0.5 > M.MACHINED_BOTTOM_RADIUS_MM + 0.000001 then
    return false, string.format(
      ...
      finish_dia, finish_dia / 25.4, M.MACHINED_BOTTOM_RADIUS_MM,
      2.0 * M.MACHINED_BOTTOM_RADIUS_MM,
      2.0 * M.MACHINED_BOTTOM_RADIUS_MM / 25.4)
  end
