...
local vbit_error
ok, vbit_error = core.validate_tool_geometry(6.35, 1.5875, 12.7, 90, 0.2, 4.65, 6)
assert(not ok, "one-half inch V-bit should fail lower-chamfer interference check")
assert(string.find(vbit_error, "gouge", 1, true), "V-bit error should explain wall gouging")
assert(string.find(vbit_error, "0.5000 in", 1, true), "V-bit error should show selected inch size")
assert(string.find(vbit_error, "1.400 mm", 1, true), "V-bit error should show lower-chamfer diameter limit")
ok = core.validate_tool_geometry(6.35, 1.5875, 12.7, 45, 0.2, 4.65, 6)
assert(not ok, "45 degree included-angle bit should fail")
ok, vbit_error = core.validate_tool_geometry(6.35, 1.5875, 1.4, 90, 0.2, 4.65, 6)
assert(not ok, "lower-chamfer-sized V-bit should fail the upper-chamfer requirement")
...
local tool_error
ok, tool_error = core.validate_tool_geometry(6.35, 3.175, 12.7, 90, 0.2, 4.65, 8)
assert(not ok, "one-eighth inch finish cutter should fail the bottom corner")
assert(string.find(tool_error, "1/16 inch", 1, true), "finish-tool error should recommend 1/16 inch")
assert(string.find(tool_error, "0.1250 in", 1, true), "finish-tool error should show selected inch size")
